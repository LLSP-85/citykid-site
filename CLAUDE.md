...
## Venue strings & the venue view (added Aug 23 2026)

Every event's `l` string is **canonical**: `Venue Name · [sub-location ·] Neighborhood (exact street address)`. The text before the first `·` is the venue key; the venue page (`#venue=<slug>`, tap any venue name on a card) groups on it, so "Kensington" and "Kensington Library" would be two venues. The venue page (added Aug 24, ohmyrockness-style) replaces the calendar with one chronological list of every upcoming date at that venue across all launched months, under the yellow venue bar; "Back to the calendar" restores the grid. It renders from the same MONTHS arrays at load time, so it needs no per-venue maintenance. Rules:

- Libraries are always `X Library` (never the bare branch name), with the branch's street address: Cortelyou Library · Ditmas Park (1305 Cortelyou Rd), Flatbush Library · Flatbush (22 Linden Blvd), Kensington Library · Kensington (4207 18th Ave), Windsor Terrace Library · Windsor Terrace (160 E 5th St), East Flatbush Library · East Flatbush (9612 Church Ave), Park Slope Library · Park Slope (431 6th Ave), Central Library · Prospect Heights (10 Grand Army Plaza). Room names go in the middle: `Kensington Library · Storytime Room · Kensington (4207 18th Ave)`.
- Always include the exact address in the trailing parentheses. Parks and open streets that have no street number get the nearest entrance cross-streets instead (`Prospect Park · Audubon Center · (enter at Lincoln Rd & Ocean Ave)`). Manhattan venues put `, Manhattan` inside the parens so `nbhdOf()` keeps routing them to the Manhattan chip.
- Copy the existing string for a venue that is already on the calendar rather than inventing a variant. A new venue gets one canonical string and one GEO pin.
- **Operation Gig**: one venue key, `Operation Gig · Ditmas Park (<house address>)` per concert. Every row links to `LNK.og` (the landing page), and the venue bar shows "location changes by date" because the addresses differ. When LL feeds new porch concerts from IG, update the event row, a GEO pin for any new house address (street-number matcher, e.g. `"330 Westminster"`), AND `operation-gig.html` in the same session.
- GEO matcher trap: adding `(10 Grand Army Plaza)` to Central Library made the generic `Grand Army Plaza` pin steal it; the Central Library row now sits at the top of GEO. Any time you add an address that contains another matcher's text, re-run the pin-name comparison (`cmp.js` pattern: assert `geo(e.l)[0]` is unchanged for every event, not just "0 misses").
- The JSON-LD `place()` parses the trailing `(digits …)` into `streetAddress`, so the address format also feeds SEO.
- Renaming an `l` string changes the event's heart id (hash of title|venue|time); saved hearts for renamed rows are lost once. Don't churn venue strings casually.
