...
export default {
  ...
  async fetch(request, env) {
    ...
    if (request.method === 'POST' && url.pathname === '/submit') {
      let info = '', honeypot = null;
      try {
        const ct = request.headers.get('content-type') || '';
        if (ct.includes('json')) {
          const j = await request.json();
          info = String(j.event_info || '');
          honeypot = 'nickname' in j ? String(j.nickname || '') : null;
        } else {
          const fd = await request.formData();
          info = String(fd.get('event_info') || '');
          const hp = fd.get('nickname');
          honeypot = hp === null ? null : String(hp);
        }
      } catch (e) {
        return new Response('Bad request', { status: 400 });
      }
      // Honeypot filled = bot filled the trap. Honeypot ABSENT = direct post that
      // never loaded the real form (the site's form always sends the field, even empty).
      // Either way: pretend success, store nothing (same pattern as /subscribe).
      if (honeypot === null || honeypot.trim()) return Response.redirect(url.origin + '/?submitted=1', 303);
    ...
    if (request.method === 'POST' && url.pathname === '/subscribe') {
      ...
      // Honeypot filled or absent = bot (see /submit). Pretend success, store nothing.
      if (honeypot === null || honeypot.trim()) return Response.redirect(url.origin + '/?subscribed=1', 303);
